...
const requirementEnum = z.enum(["PHOTO", "DOCUMENT"]);
const scheduleEnum = z.enum(["DAILY", "WEEKLY", "MONTHLY", "CUSTOM"]);

export type ProgramRequirementType = z.infer<typeof requirementEnum>;
export type ProgramScheduleType = z.infer<typeof scheduleEnum>;
...
export const programBaseSchema = z.object({
  ...
  requirementType: requirementEnum.default("PHOTO"),
  minUploads: z.number().min(1).max(10).default(1),
...
export const programUpdateSchema = z.object({
  ...
  scheduleTime: z
    ...
    .regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, "Format waktu tidak valid")
    .optional(),
  requirementType: requirementEnum.optional(),
  minUploads: z.number().min(1).max(10).optional(),
